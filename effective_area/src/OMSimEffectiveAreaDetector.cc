...
*/
    ...
    G4Box *Photocathode = new G4Box("Photocathode", 20 * cm, 20 * cm, lPhotocathodeThickness);  // Construct a box with name, and half lengths pX,pY,pZ
    ...
    G4LogicalVolume *PhotocathodeLog = new G4LogicalVolume(Photocathode, mData->getMaterial("RiAbs_NicoPalladium"), "Photocathode");
    G4LogicalVolume *GlassLog = new G4LogicalVolume(Glass, mData->getMaterial("RiAbs_NicoGlass"), "Glass");
    G4LogicalVolume *VacuumLog = new G4LogicalVolume(Vacuum, mData->getMaterial("Ri_Vacuum"), "Vacuum");
    ...
    NicoSurface->SetFinish(polished);
    
    G4MaterialPropertiesTable* lTable = mData->getMaterial("RiAbs_NicoPalladium")->GetMaterialPropertiesTable();
    lTable->AddConstProperty("COATEDTHICKNESS", 20*nm);
    NicoSurface->SetMaterialPropertiesTable(lTable);
    ...
    /*  G4PVPlacement* PhotocathodePhys = new G4PVPlacement( //photocathode as a 3-dim volume in the center
         0,
         G4ThreeVector(0, 0, -lGlassThickness-lPhotocathodeThickness),
         PhotocathodeLog,
         "Photocathode",
         mWorldLogical,
         false,
    ...
    G4PVPlacement *VacuumPhys = new G4PVPlacement(
        0,
        G4ThreeVector(0, 0, -lGlassThickness-lVacThinkness), // Vacuum is directly underneath the Glass
        VacuumLog,
        "PMTvacuum",
        mWorldLogical,
        false,
    ...
    new G4LogicalBorderSurface("GlassToVacuum", GlassPhys, VacuumPhys, NicoSurface); // photocathode as a 2-dim border in between glass and vacuum
    new G4LogicalBorderSurface("VacuumToGlass", VacuumPhys, GlassPhys, NicoSurface);

    // Placements of the absorbers on top and bottom to count reflections and transmissions
    new G4PVPlacement(0, G4ThreeVector(0, 0, 30 * cm), TopAbsorberLog, "TopAbsorber", mWorldLogical, false, 0, true);
    new G4PVPlacement(0, G4ThreeVector(0, 0, -(3 * lVacThinkness )), BottomAbsorberLog, "BottomAbsorber", mWorldLogical, false, 0, true);
} 
